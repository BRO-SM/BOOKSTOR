...
async function handleLogin() {
  ...
  try {
    ...
    if (
      ...
      hashedPassword === (await hashPassword("1111"))
    ) {
      isLoggedIn = true;
      ...
      document
        .querySelectorAll(".delete-book-btn")
    ...
    Swal.fire({
      ...
      footer:
        "The correct information is: Email: admin@gmail.com, Password: 1111",
      timer: 4000,
      showConfirmButton: false,
    });
  }
...
function showBookDetails(book) {
  ...
  document.getElementById("bookDetailsLongSummary").textContent =
    book.longSummary;
  ...
  return `
    ...
    book.title
                ...
                <div class="card-body">
                    ...
                    <p class="card-text text-bg-warning"><small class="text-muted">Author: ${
                      book.author
                    }</small></p>
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <button class="btn btn-info" onclick="showBookDetails(${JSON.stringify(
                          book
                        ).replace(/"/g, "&quot;")})">
                            Read More
                        </button>
...
function updateCartDisplay() {
  ...
  if (cart.length > 0) {
    ...
    totalElement.className =
      "dropdown-item font-weight-bold border-top mt-2 pt-2";
    totalElement.innerHTML = `Total: $${total.toFixed(2)}`;
    cartItems.appendChild(totalElement);
    ...
    cartItems.innerHTML =
      '<div class="dropdown-item text-center">Your cart is empty</div>';
  }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (subscribeForm) {
    subscribeForm.addEventListener("submit", function (event) {
      event.preventDefault();
      const firstName = subscribeForm.querySelector(
        'input[name="firstName"]'
      )?.value;
      const lastName = subscribeForm.querySelector(
        'input[name="lastName"]'
      )?.value;
